...
export default function ContactPage({ setCurrentPage, setActiveSection }: ContactPageProps) {
  ...
  return (
    <div className="min-h-screen pt-28 pb-24 bg-[#030303] text-white overflow-hidden relative">
      ...
      <div className="max-w-5xl mx-auto px-4 md:px-8 relative z-10">
        <button
          onClick={goHome}
          className="group inline-flex items-center gap-2 text-zinc-500 hover:text-pink-400 text-xs font-mono mb-16 transition-colors cursor-pointer"
        >
        ...
        <section className="text-center mb-10">
          ...
          <p className="mt-5 text-[11px] md:text-xs font-mono uppercase tracking-[0.35em] text-zinc-500">
            ...
            {socials.map((social) => {
              ...
              return (
                <a
                  ...
                  rel="noreferrer"
                  title={social.name}
                  className="h-10 w-10 rounded-full bg-zinc-950 border border-zinc-900 text-zinc-500 hover:text-white hover:border-pink-500/40 hover:bg-zinc-900 flex items-center justify-center transition-all"
                >
        ...
        <div className="flex justify-center mb-8">
          <div className="p-1 rounded-full bg-zinc-950 border border-zinc-900 flex items-center gap-1">
            <button
              onClick={() => setActiveTab('call')}
              className={`flex items-center gap-2 px-5 py-2.5 rounded-full text-xs font-bold transition-all cursor-pointer ${activeTab === 'call' ? 'bg-white text-black' : 'text-zinc-400 hover:text-white'}`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 px-5 py-2.5 rounded-full text-xs font-bold transition-all cursor-pointer ${activeTab === 'message' ? 'bg-white text-black' : 'text-zinc-400 hover:text-white'}`}
            >
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'message' ? (
            <motion.form
              ...
              className="max-w-3xl mx-auto rounded-3xl border border-zinc-900 bg-zinc-950/70 p-6 md:p-8 shadow-2xl backdrop-blur"
            >
              {status === 'success' ? (
                <div className="py-16 text-center">
                  ...
                  <p className="text-sm text-zinc-500 max-w-sm mx-auto mb-7">Thanks. Your message was received and I will get back to you shortly.</p>
                  <button
                    type="button"
                ...
                <>
                  <div className="grid md:grid-cols-2 gap-5">
                    <label className="space-y-2">
                      <span className="block text-[10px] font-mono text-zinc-500 uppercase tracking-[0.18em] font-bold">Name</span>
                      <input
                        value={formData.name}
                        onChange={(event) => setFormData({ ...formData, name: event.target.value })}
                        placeholder="Jane Doe"
                        className="w-full rounded-xl bg-black border border-zinc-900 px-4 py-3 text-sm text-white placeholder:text-zinc-700 focus:outline-none focus:border-pink-500/70"
                      />
                    </label>
                    <label className="space-y-2">
                      <span className="block text-[10px] font-mono text-zinc-500 uppercase tracking-[0.18em] font-bold">Email</span>
                      <input
                        type="email"
                  ...
                  <div className="mt-6">
                    <span className="block text-[10px] font-mono text-zinc-500 uppercase tracking-[0.18em] font-bold mb-3">Topic</span>
                    <div className="flex flex-wrap gap-2">
                      {topics.map((topic) => (
                        <button
                          ...
                          className={`px-4 py-2 rounded-full border text-xs font-semibold transition-all cursor-pointer ${formData.topic === topic ? 'bg-pink-500 border-pink-500 text-white' : 'bg-black border-zinc-900 text-zinc-400 hover:text-white hover:border-zinc-700'}`}
                        >
                          {topic}
                        </button>
                      ))}
                  ...
                  <label className="block mt-6 space-y-2">
                    <span className="block text-[10px] font-mono text-zinc-500 uppercase tracking-[0.18em] font-bold">Message</span>
                    <textarea
                      rows={6}
                      value={formData.message}
                      onChange={(event) => setFormData({ ...formData, message: event.target.value })}
                      placeholder="Tell me about your project, idea, or just say hi..."
                      className="w-full rounded-xl bg-black border border-zinc-900 px-4 py-3 text-sm text-white placeholder:text-zinc-700 focus:outline-none focus:border-pink-500/70 resize-none"
                    />
                  </label>
                  ...
                  <label className="mt-5 flex items-start gap-3 text-xs text-zinc-500 cursor-pointer">
                    <input
                      ...
                      className="mt-0.5 h-4 w-4 rounded border-zinc-800 bg-black accent-pink-500"
                    />
